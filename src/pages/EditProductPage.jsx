...
import { AlertCircle, ArrowLeft, DollarSign, Image as ImageIcon, Loader2, Package, Plus, Tag, Trash2, Truck, Upload, X } from "lucide-react"
...
const EditProductPage = () => {
  ...
  // Detectar se a imagem é URL ou base64
      ...
      try {
        ...
        if (result.success) {
          ...
          if (imgType === "base64") {
            ...
            // Para URL, você pode escolher não pré-carregar a imagem grande
            // mas aqui mostramos a mesma como preview também
  ...
  const handleDeleteProduct = async () => {
    if (!window.confirm("Tem certeza de que deseja excluir este produto? Essa ação não pode ser desfeita.")) return
    setIsDeleting(true)
    try {
  ...
  const processFile = (file) => {
    ...
    setImageFile(file)
    setImageType("file")
    setFormData((prev) => ({ ...prev, imageMain: "" })) // evitar conflito com URL existente

    const reader = new FileReader()
    reader.onloadend = () => setImagePreview(reader.result)
    reader.readAsDataURL(file)

    if (errors.imageMain) setErrors((prev) => ({ ...prev, imageMain: "" }))
  }
  ...
  // Upload puro: recebe File e retorna string (URL/base64) ou null
  const handleUploadImage = async (file) => {
    ...
    try {
      ...
      const uploaded =
        response?.success && response.data && (response.data.url || response.data.base64) ? response.data.url || response.data.base64 : null

      if (uploaded) {
        setFormData((prev) => ({ ...prev, imageMain: uploaded }))
        setImagePreview(uploaded) // preview = dado salvo
        setImageType(uploaded.startsWith("data:image") ? "base64" : "url")
        toast({ title: "Sucesso", description: "Imagem enviada com sucesso" })
        return uploaded
      }

      const msg = (response && (response.message || response.data?.error)) || "Falha no upload da imagem."
      toast({ title: "Erro", description: msg, variant: "destructive" })
      return null
    } catch (error) {
      console.error("Erro ao fazer upload:", error)
      const msg = error?.message || "Erro ao fazer upload da imagem. Tente novamente."
      toast({ title: "Erro", description: msg, variant: "destructive" })
      return null
    } finally {
  ...
  const validateForm = () => {
    ...
    if (!imageFile && !formData.imageMain.trim()) newErrors.imageMain = "Imagem principal é obrigatória"
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        toast({ title: "Sucesso", description: "Produto atualizado com sucesso", duration: 3000 })
        navigate(`/produto/${id}`)
      }
    } catch (error) {
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center py-12">
          ...
          <h2 className="text-2xl font-semibold text-gray-900 mb-2">Produto não encontrado</h2>
      ...
      <div className="max-w-4xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
          ...
          <div className="flex items-center gap-2 text-sm text-gray-600 mb-6">
            <button onClick={() => navigate(`/produto/${id}`)} className="hover:text-gray-900 flex items-center gap-1">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex justify-end">
                  <Button variant="destructive" disabled={isDeleting} onClick={handleDeleteProduct}>
                    {isDeleting ? (
                      <>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  {!imagePreview ? (
                    <div
                      ...
                      onDrop={handleDrop}
                      className={`border-2 border-dashed rounded-lg p-8 text-center cursor-pointer transition-colors ${errors.imageMain ? "border-red-500 bg-red-50" : "border-gray-300 hover:border-gray-400 bg-gray-50"}`}
                      onClick={() => fileInputRef.current?.click()}
                    >
                      <input ref={fileInputRef} id="imageUpload" type="file" accept="image/*" onChange={handleFileSelect} className="hidden" />
                    ...
                    <div className="relative border-2 border-gray-300 rounded-lg p-4">
                      <img src={imagePreview} alt="Preview" className="w-full h-64 object-contain rounded" />
                      <Button type="button" variant="destructive" size="sm" className="absolute top-2 right-2" onClick={handleRemoveSelectedImage}>
                        <X className="h-4 w-4" />
                      </Button>
                      {isUploading && (
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      handleChange(e)
                      if (e.target.value && imagePreview && imageFile) {
                        setImageFile(null)
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      {formData.images.map((image, index) => (
                        <Badge key={index} variant="secondary" className="flex items-center gap-1">
                          Imagem {index + 1}
                          <button type="button" onClick={() => handleRemoveImage(image)} className="ml-1 hover:text-red-500">
                            <X className="h-3 w-3" />
                          </button>
                        </Badge>
                      ))}
                ...
                {formData.features.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {formData.features.map((feature, index) => (
                      <Badge key={index} variant="secondary" className="flex items-center gap-1">
                        {feature}
                        <button type="button" onClick={() => handleRemoveFeature(feature)} className="ml-1 hover:text-red-500">
                          <X className="h-3 w-3" />
                        </button>
                      </Badge>
                    ))}
            ...
            <div className="flex justify-end space-x-4">
              <Button type="button" variant="outline" onClick={() => navigate(`/produto/${id}`)} disabled={isSubmitting}>
                Cancelar
              </Button>
